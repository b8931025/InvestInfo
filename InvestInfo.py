...
def get3People(sToday):
    ...
    #自營商(自行買賣)
    s0,s1 = df.iloc[0,0],billion(df.iloc[0,3])
    #投信
    s2,s3 = df.iloc[2,0],billion(df.iloc[2,3])
    #外資
    s4,s5 = df.iloc[3,0],billion(df.iloc[3,3])
    #買賣超合計
    s6,s7 = df.iloc[5,0],billion(df.iloc[5,3])

    #輸出格式
    sFormat = f'''
    <table style='border:1px solid black;margin-left:auto;margin-right:auto;padding:5px;' width='80%'
...
def getFuture():
    ...
    df = pd.read_html(sUrl)[2].iloc[4:8,0:17].iloc[1:,[2,9,10,11,12,13,14]]
    df.reset_index(drop=True, inplace=True)
    df.columns = ['身份別','多方口數','多方淨額','空方口數','空方淨額','多空口數','多空淨額']

    #千分位
    for i in range(0,3):
        for j in range(1,7): df.iloc[i,j] = "{0:,}".format(int(df.iloc[i,j]))
    ...
    sOut = f'''<br>
        ...
        rules="all" cellpadding="5" width="80%">
...
if __name__ == '__main__':
    ...
    sToday = dToday.strftime('%Y/%#m/%#d') #yyyy/mm/dd 不補0
    sToday2 = dToday.strftime('%Y%m%d') #yyyymmdd 補0
    ...
    if isHoliday(schd,sToday):
        ...
        #大盤
        sBody = getXTAI(sToday2)
        #三大法人
        sBody += get3People(sToday2)
        #台指期外資未平倉淨額
        sBody += getFuture()
